...
v2 changes (this file), relative to the frozen primary_pipeline_v1.py:
  - PRETTY labels updated to the corrected construct terminology (agreed with A. Esbensen,
    Aug 2026): TROGB -> "Grammatical comprehension (TROG-2)", NonLRAE -> "Nonword serial
    recall", NIQ -> "Nonverbal reasoning (Block Design)", LSPLRAE -> "CLPT listening span",
    GåStop -> "Response inhibition (Go/Stop)", HimOpmscore_skala -> "Selective attention
    (Sky Search)", OOU -> "Visuospatial WM (Odd-One-Out)". These appear in figure labels
    and in the `label` column of results/feature_importance.csv.
  - fig_importance LASSO-panel labels moved to the outer edge to avoid overlap with the
    (negative) coefficient bars.
  - New output: results/oof_predictions.csv (out-of-fold probabilities, previously computed
    but discarded), so ROC/confusion/calibration can be replotted without refitting.
Model, CV, seed, and data are unchanged, so all numeric results are identical to v1.
...
Outputs (overwrite; confirmatory now that the set is locked):
  ...
  results/oof_predictions.csv      (out-of-fold LASSO probabilities; feeds ROC/CM/calibration replots)
...
PRETTY = {"LSPHUK":"CLPT word recall","LTaF":"Digit span forward","TROGB":"Grammatical comprehension (TROG-2)",
          "LSPLRAE":"CLPT listening span","GåStop":"Response inhibition (Go/Stop)","verbfluantal":"Verbal fluency: correct",
          "verbfluskift":"Verbal fluency: switches","HimOpmscore_skala":"Selective attention (Sky Search)",
          "NonLRAE":"Nonword serial recall","LTaB":"Digit span backward","NIQ":"Nonverbal reasoning (Block Design)",
          "verbflusubkat":"Verbal fluency: subcat.","OOU":"Visuospatial WM (Odd-One-Out)","verbfluintru":"Verbal fluency: intrusions",
...
pd.DataFrame({"y_true": y, "oof_prob": oof_prob}).to_csv(f"{RESULTS}/oof_predictions.csv", index=False)
...
ax.set_xlabel("Mean predicted probability (DLD)")
ax.set_ylabel("Observed fraction with DLD")
...
# One ordering for both panels: RF_SHAP descending, reversed so rank 1 sits at the top.
d = imp.iloc[::-1]
ypos = np.arange(len(d))

ax[0].barh(ypos, d["RF_SHAP"], color="#4c72b0")
ax[0].set_yticks(ypos)
ax[0].set_yticklabels(d["label"], fontsize=9)
#ax[0].set_title(f"A. RF importance ({'mean |SHAP|' if shap_ok else 'impurity'})")
ax[0].set_title("A", loc="left", fontsize=11, fontweight="bold")
ax[0].set_xlabel("mean |SHAP| (probability)")

cols = ["#c44e52" if v < 0 else "#55a868" for v in d["LASSO_coef"]]
ax[1].barh(ypos, d["LASSO_coef"], color=cols)
ax[1].axvline(0, color="k", lw=0.8)
#ax[1].set_title(f"B. LASSO coefficients (C={best_C:.3g})")
ax[1].set_title("B", loc="left", fontsize=11, fontweight="bold")
...
#ax.set_title("Minimal screening battery"); 
