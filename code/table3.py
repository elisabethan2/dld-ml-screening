...
Outputs:
  results/table1_demographics2108.csv
  results/table3_group_comparison2108.csv
...
PRETTY = {"LSPHUK":"CLPT word recall","LTaF":"Digit span forward",
          "TROGB":"Grammatical comprehension (TROG-2)","LSPLRAE":"CLPT listening span",
          "GåStop":"Response inhibition (Go/Stop)","verbfluantal":"Verbal fluency: correct",
          "verbfluskift":"Verbal fluency: switches","HimOpmscore_skala":"Selective attention (Sky Search)",
          "NonLRAE":"Nonword serial recall","LTaB":"Digit span backward",
          "NIQ":"Nonverbal reasoning (Block Design)","verbflusubkat":"Verbal fluency: subcat.",
          "OOU":"Visuospatial WM (Odd-One-Out)","verbfluintru":"Verbal fluency: intrusions",
...
pd.DataFrame(t1).to_csv(f"{RESULTS}/table1_demographics2108.csv",index=False)
...
for f in FEATURES:
    ...
    rows.append({"Measure":PRETTY[f],"variable":f,
        "n":len(g['DLD'])+len(g['HI'])+len(g['Control']),
        "DLD_M_SD":f"{g['DLD'].mean():.2f} ({g['DLD'].std(ddof=1):.2f})",
        "HI_M_SD":f"{g['HI'].mean():.2f} ({g['HI'].std(ddof=1):.2f})",
        "Control_M_SD":f"{g['Control'].mean():.2f} ({g['Control'].std(ddof=1):.2f})",
...
cols=["Measure","variable","n","DLD_M_SD","HI_M_SD","Control_M_SD","n_test","KW_H","KW_p","KW_p_holm15","epsilon2",
      "Dunn_DLDvsControl_p_holm","Dunn_DLDvsHI_p_holm","Dunn_HIvsControl_p_holm"]
pd.DataFrame(rows)[cols].to_csv(f"{RESULTS}/table3_group_comparison2108.csv",index=False)
...
print("Wrote results/table1_demographics2108.csv, table3_group_comparison2108.csv (+ run_log.md).")
